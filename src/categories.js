...
// The mapping below follows what is seen on site; confirm with a tradesperson
// before release (backlog h11). APWA: the US utility-locating code.
...
export const SCHEMES = {
  it: {
    gas: "#F2C300", electric: "#2B2B2B", data: "#2E9B4F", alarm: "#7A4B2A",
    intercom: "#6EC1E4", multimedia: "#7B3FA0", cold_water: "#1B62B5",
    hot_water: "#D42B1E", drain: "#4F7F4F", other: "#8A8F8A",
  },
  apwa: {
...
export function pinIcon(colour, { selected = false } = {}) {
  const stroke = selected ? "#ffffff" : "rgba(0,0,0,0.35)";
  const w = selected ? 2.5 : 1.25;
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="26" height="36" viewBox="0 0 26 36">
    ...
    <circle cx="13" cy="12.8" r="4.2" fill="rgba(255,255,255,0.85)"/>
  </svg>`;
